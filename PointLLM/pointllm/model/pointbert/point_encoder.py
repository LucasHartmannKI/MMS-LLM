...
class PointTransformer(nn.Module):
    ...
    def forward(self, pts):
        ...
        group_input_tokens = self.encoder(neighborhood)  # B: bathsize G：number of group N: number of feature in each group
        group_input_tokens = self.reduce_dim(group_input_tokens) # * B, G, C(384)
        # prepare cls
        cls_tokens = self.cls_token.expand(group_input_tokens.size(0), -1, -1) # * B, 1, C(384)
        cls_pos = self.cls_pos.expand(group_input_tokens.size(0), -1, -1) # * B, 1, C(384)
        # add pos embedding
        pos = self.pos_embed(center) # * B, G, C(384)
        # final input
        x = torch.cat((cls_tokens, group_input_tokens), dim=1) # * B, G + 1(cls token)(513), C(384)
        pos = torch.cat((cls_pos, pos), dim=1) # * B, G + 1(cls token)(513), C(384)
        # transformer
        x = self.blocks(x, pos) # * B, G + 1(cls token)(513), C(384)
        x = self.norm(x) # * B, G + 1(cls token)(513), C(384)
        if not self.use_max_pool:
            return x  # * B, G + 1(cls token)(513), C(384)
