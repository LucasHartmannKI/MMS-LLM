...
class Encoder(nn.Module):
    ...
    def forward(self, point_groups):
        '''
            point_groups : B G N 3 /batchsize, group, num_points of every group, 3
            -----------------
            feature_global : B G C  /batchsize, group, Global feature  for each point group
        '''
        bs, g, n, c = point_groups.shape # B:1 G:512 N:32 C:3
        point_groups = point_groups.reshape(bs * g, n, c)
        # encoder
        feature = self.first_conv(point_groups.transpose(2, 1))  # BG c:256 n    first convlayer, get local feature
        feature_global = torch.max(feature, dim=2, keepdim=True)[0]  # BG c:256 1    maxpooling, get global feature
        feature = torch.cat([feature_global.expand(-1, -1, n), feature], dim=1)  # BG 512 n   concat global feature with local feature
        feature = self.second_conv(feature)  # BG 1024 n     second convlayer, get feature
        feature_global = torch.max(feature, dim=2, keepdim=False)[0]  # BG 1024   second maxpooling, get global feature
        return feature_global.reshape(bs, g, self.encoder_channel) # B G C /1 512 1024
