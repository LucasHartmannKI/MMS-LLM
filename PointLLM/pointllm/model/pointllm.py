...
class PointLLMConfig(LlamaConfig): # * inherit from LlamaConfig
    model_type = "pointllm"

class PointLLMLlamaModel(LlamaModel): #use for extracting features
    config_class = PointLLMConfig 

    def __init__(self, config: LlamaConfig):
        super(PointLLMLlamaModel, self).__init__(config) # * call the parent class's constructor
        ...
        backbone_output_dim = self.point_backbone_config["backbone_output_dim"] # * 384
        ...
        if self.point_backbone_config['projection_hidden_layer'] > 0:
            ...
            projection_layers = []
            last_dim = backbone_output_dim # * 384
            for i in range(point_bert_config.model.projection_hidden_layer):
                ...
                last_dim = self.point_backbone_config["projection_hidden_dim"][i] #projection_hidden_dim: [1024, 2048]
    ...
    def forward(
        ...
        if point_backbone is not None and (input_ids.shape[1] != 1 or self.training) and point_clouds is not None:
            ...
            with torch.no_grad() if self.fix_pointnet else nullcontext():
                ...
                if type(point_clouds) is list:
                    ...
                    for point_cloud in point_clouds: # * iterate over batch
                        ...
                        point_feature = self.point_backbone(point_cloud) #use PointTransformer to extract features (1, 513, 384)
                        print(f'point feature:{point_feature.shape}') # 
                        point_features.append(point_feature)
                else:
                    point_features = self.point_backbone(point_clouds)

            if type(point_clouds) is list: # * variable numbers of points
                point_features = [self.point_proj(point_feature) for point_feature in point_features] # * project the features 384->4096
                print(f'point_features after projection:{point_features[0].shape}') # torch.Size([1, 513, 4096])
            else:
            ...
            print(f'dummy_point_features:{dummy_point_features.shape}') # torch.Size([513, 4096])

            new_input_embeds = []
            cur_point_idx = 0
            #print(point_features[0].shape) # 1, G + 1(cls token)(513), 4096
            if type(point_features) is list:
                point_features = torch.cat(point_features, dim=1)
                print(point_features.shape) # B, (513)*l, 4096 
            #point_features = point_features[0] if type(point_clouds) is list else point_features #---------------------------
            for cur_input_ids, cur_input_embeds in zip(input_ids, inputs_embeds): # * input_ids: B, L; input_embeds: B, L, C  token ID 和嵌入向量
                if (cur_input_ids == point_backbone_config['point_patch_token']).sum() == 0: #if there is no point token
                    # multimodal LLM, but the current sample is not multimodal
                    cur_input_embeds = cur_input_embeds + (0. * dummy_point_features).sum() # * do nothing
                    new_input_embeds.append(cur_input_embeds)
                    cur_point_idx += 1
                    continue
                cur_point_features = point_features[cur_point_idx].to(device=cur_input_embeds.device) # (513)*l, 4096 
                num_patches = cur_point_features.shape[0] # * number of point tokens 2052=(513)*l
                if point_backbone_config['mm_use_point_start_end']:
                    ...
                    print(f'point_start_tokens:{point_start_tokens}')  #tensor([34], device='cuda:0')
                    ...
                    print(f'cur_new_input_embeds:{cur_new_input_embeds.shape}') #torch.Size([2094, 4096]) 33+2052+9=2094
                    print(f'new_input_embeds:{len(new_input_embeds)}') # 1
                else:
                    ...
                    new_input_embeds.append(cur_new_input_embeds)
                    cur_point_idx += 1
            inputs_embeds = torch.stack(new_input_embeds, dim=0)  #torch.Size([1, 2094, 4096])
...
class PointLLMLlamaForCausalLM(LlamaForCausalLM): # * inherit from LlamaForCausalLM，used for text generation
    config_class = PointLLMConfig
        ...
        model_inputs.update(
            {
                ...
                "attention_mask": attention_mask,
                "point_clouds": kwargs.get("point_clouds", None), # * add point_clouds
            }
        )
        return model_inputs
