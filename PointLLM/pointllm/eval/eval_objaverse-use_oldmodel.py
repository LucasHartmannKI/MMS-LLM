...
PROMPT_LISTS = [
    "Please tell me its category only. (No more than 3 words)",#What is this?
...
def init_model(args):
    # model_name = os.path.expanduser(args.model_name)
    # print(f'[INFO] Using local model: {os.path.basename(model_name)}')
    # tokenizer = AutoTokenizer.from_pretrained(model_name)
...
def main(args):
    # * ouptut
    args.output_dir = os.path.join('/home/test_oldmodel', "evaluation")
    ...
    if not os.path.exists(args.output_file_path):
        ...
        if args.prompt_index != 0 and args.prompt_index != 1:
            ...
            annos = {anno["object_id"]: anno["object_id"].rsplit('_', 1)[0].replace('_', ' ') for anno in annos}
...
if __name__ == "__main__":
    ...
    # * dataset type
    parser.add_argument("--data_path", type=str, default="/data/large/ikg-data/instance_seg_c17_reflectivity_npy", required=False)
    parser.add_argument("--anno_path", type=str, default="/home/PointLLM/data/anno_data/ikgc17_brief_description_val.json", required=False)
