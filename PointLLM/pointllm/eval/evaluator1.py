        ...
        try:
            with Pool(num_workers) as pool:
                with tqdm(total=len(results)) as pbar:
                    for result in pool.imap_unordered(self.evaluate_result, results):
                        if result is None:
                            print("Invalid result encountered. Skipping...")
                            continue  # 跳过无效结果
                        
                        # 解构返回值
                        object_id, model_output, ground_truth, accuracy, cls_result, reason, prompt_tokens, completion_tokens = result
...
class OpenAIObjectCaptioningEvaluator(OpenAIOpenFreeFormClsEvaluator):
    ...
    def evaluate_result(self, result):
        ...
        try:
            ...
            # 增加日志，检查 gpt_response 内容
            if not gpt_response or "choices" not in gpt_response or not gpt_response['choices']:
                raise ValueError(f"Invalid GPT response: {gpt_response}")
            
            prompt_tokens = gpt_response['usage']['prompt_tokens']
            completion_tokens = gpt_response['usage']['completion_tokens']
            content = gpt_response['choices'][0]["message"]['content']
    
            accuracy, cls_result, reason = self.parse_gpt_response_evaluate(content)
            
            return object_id, model_output, ground_truth, accuracy, cls_result, reason, prompt_tokens, completion_tokens
        except Exception as e:
            print(f"Error in evaluate_result for object_id {object_id}: {e}")
            return None
    ...
    def safe_evaluate_result(args):
        try:
            return evaluator.evaluate_result(args)
        except Exception as e:
            return {"error": str(e)}
    ...
    def parallel_evaluate(self, num_workers=20):
        ...
        try:
            with Pool(num_workers) as pool:
                with tqdm(total=len(results)) as pbar:  # create a progress bar
                    for result in pool.imap_unordered(safe_evaluate_result, results):
                        if "error" in result:
                            print(f"[ERROR] {result['error']}")
                            continue
                        # 处理正常结果
                        self.correct_predictions += result['accuracy']
                        self.total_predictions += 1
                        ...
                        self.response_data.append({
                            ...
                            'ground_truth': result['ground_truth'],
                            'model_output': result['model_output'],
                            'gpt_cls_result': result['cls_result'],
            ...
            print(f"Error {e} occurred during parallel evaluation.")
            self.save_results(is_temp=True)
            exit()
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Switching to single-threaded evaluation.")
        start_evaluation(results=args.results_path, output_dir=args.output_dir, output_file=output_file, 
                         eval_type=args.eval_type, model_type=args.model_type, 
                         parallel=False)
