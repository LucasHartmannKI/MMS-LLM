...
def load_ikg_point_cloud(data_path, object_id,  use_color=False):
    filename = f"{object_id}.npy"

    point_cloud = np.load(os.path.join(data_path, filename))

    labels = point_cloud[:, 7]         # Column 8 for the label

    # * normalize
    point_cloud = pc_norm(point_cloud[:, :7])

    # Get all unique labels
    unique_labels = np.unique(labels)
    #num_unique_labels = len(unique_labels)
    # Iterate through each unique label, extract points, and save to a separate file
    list_of_instances = []
    for label in unique_labels:
        # Extract points with the current label
        if label == 0:
            continue 
        instance = point_cloud[labels == label] 
        if not use_color:
            instance = instance[:, :3]
        instance = torch.from_numpy(instance).unsqueeze(0).to(torch.float16)
